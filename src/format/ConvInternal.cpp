...
namespace internal
{
	...
	\param [out] ctx Here resulting data is written
	\param n The number to convert
	\param digits The maximal number of digits after the decimal-point.
	\param forcePrecision Force the number of digits and fill with zeros if necessary
	\param locale The locale facet to use for output
	...
	*/
	...
	//! Write a number in hexadecimal floating point notation
...
Slice ConvertString(Context& dst, StringType srcType, const char* srcData, size_t srcSize)
{
	...
	if(srcType == StringType::Unicode) {
		if(dst.stringType == StringType::Ascii) {
			...
			char* base = data;
			while(s < e) {
				if((*s & 0xC0) != 0x80) // Only look at the lowest byte in the utf8 sequence
					*data++ = ((*s&(uint8_t)0x80) == 0) ? (*s&(0x7F)) : '?'; // Replace non-ascii characters with a question mark
				++s;
			}
