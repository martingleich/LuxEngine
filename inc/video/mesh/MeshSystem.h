...
public:
	...
	*/
	LUX_API StrongRef<Mesh> CreatePlaneMesh(float sizeX = 1.0f, float sizeY = 1.0f, s32 tesX = 2, s32 tesY = 2, float texX = 1.0f, float texY = 1.0f, float(*function)(void* ctx, float x, float y) = nullptr, void* context = nullptr);
	...
	*/
	LUX_API StrongRef<Mesh> CreateSphereMesh(float radius = 1.0f, s32 rings = 16, s32 sectors = 32, float texX = 1.0f, float texY = 1.0f, bool inside = false);
