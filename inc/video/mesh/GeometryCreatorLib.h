...
public:
	...
	*/
	virtual StrongRef<Mesh> CreateMesh(const string& name, const core::PackagePuffer& params) = 0;
	...
	*/
	virtual StrongRef<Mesh> CreatePlaneMesh(float sizeX=1.0f, float sizeY=1.0f, s32 tesX=2, s32 tesY=2, float texX=1.0f, float texY=1.0f, float(*function)(void* ctx, float x, float y) = nullptr, void* context = nullptr) = 0;
	...
	*/
	virtual StrongRef<Mesh> CreateCubeMesh( float sizeX=1.0f, float sizeY=1.0f, float sizeZ=1.0f, s32 tesX=2, s32 tesY=2, s32 tesZ=2, float texX=1.0f, float texY=1.0f, float texZ=1.0f, bool inside=false) = 0;
	...
	\return The created plane
	*/
	virtual StrongRef<Mesh> CreateArrowMesh(float shaft_height=2.0f, float head_height=2.0f, float shaft_radius=0.4f, float head_radius=1.0f, s32 sectors=32) = 0;
};
